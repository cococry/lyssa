    ...
    if(is_hovered && lf_mouse_button_went_down(GLFW_MOUSE_BUTTON_LEFT)) {
        if(click_color) {
            ...
            if(click_color.a > 1.0)
                click_color.a = 1.0;
            lf_rect_render(pos, size, click_color);
        } else {
    ...
    if(is_hovered && (!lf_mouse_button_went_down(GLFW_MOUSE_BUTTON_LEFT) && !lf_mouse_button_is_down(GLFW_MOUSE_BUTTON_LEFT))) {
        if(hover_color) {
            ...
            if(hover_color.a > 1.0)
                hover_color.a = 1.0;
            lf_rect_render(pos, size, hover_color);
        } else {
            lf_rect_render(pos, size, color);
        }
        return LF_HOVERED;
...
void lf_image(LfTexture tex) {
    ...
    // Rendering the image
    lf_image_render(state.pos_ptr, color, tex);
...
void lf_checkbox(const char* text, bool* val, uint32_t tex) {        
    ...
    if(*val) {
        // Render the image
        lf_image_render(state.pos_ptr, (vec4s){LF_WHITE}, (LfTexture){.id = tex, .width = (uint32_t)checkbox_size, .height = (uint32_t)checkbox_size});
    }
...
void lf_rect(float width, float height, vec4s color) {
    // Rendering the rect
    lf_rect_render(state.pos_ptr, (vec2s){width, height}, color);
...
int32_t lf_menu_item_list(const char** items, uint32_t item_count, int32_t selected_index, vec4s selected_color, bool vertical) {
    ...
    next_line_on_overflow(
        (vec2s){width + padding * 2.0f + margin_right + margin_left * 2.0f, 
                    font.font_size + padding * 2.0f + margin_bottom + margin_top * 2.0f});
    ...
    for(uint32_t i = 0; i < item_count; i++) {
        LfClickableItemState item = clickable_item((vec2s){state.pos_ptr.x + element_width, state.pos_ptr.y + margin_top}, (vec2s){text_props[i].width + padding * 2.0f, (float)font.font_size + padding * 2.0f}, props, 
                                                   (i == selected_index) ? selected_color : props.color, 
                                                   0, true, true);
        if(item == LF_CLICKED) {
            clicked_item = i;  
        } 

        lf_text_render((vec2s){state.pos_ptr.x + padding + element_width, state.pos_ptr.y + margin_top + ((font.font_size + padding * 2.0f - text_props[i].height) / 2.0f)}, 
                                                items[i], font, -1, -1, -1, false, props.text_color);
        element_width += text_props[i].width + padding * 2.0f;
    } 

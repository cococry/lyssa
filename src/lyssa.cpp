...
struct Playlist {
    ...
    bool ordered = !ASYNC_PLAYLIST_LOADING;
    ...
    if(state.playlists.empty()) {
        ...
        for(auto& playlist : state.playlists) {
            ...
            if(lf_get_current_div().interact_state == LF_CLICKED && lf_get_current_div().id == lf_get_selected_div().id && !overButton) {
                state.currentPlaylist = playlistIndex;
                if(playlist.musicFiles.empty()) {
                    if(!state.loadedPlaylistFilepaths.empty())
                    ...
                    state.playlistFileThumbnailIndex = 0;
...
void renderCreatePlaylist() {
    ...
    // Form Input
    {
        ...
        inputFieldStyled(&state.createPlaylistTab.nameInput.input);
        lf_next_line();
    }
    {
        lf_next_line();
        state.createPlaylistTab.descInput.input.width = state.winWidth / 2.0f;
        inputFieldStyled(&state.createPlaylistTab.descInput.input);
        lf_next_line();
    }
            ...
            SoundFile& file = currentPlaylist.musicFiles[i];
            {
                ...
                if(hoveredTextDiv)
                {
                    ...
                    if(playButton == LF_CLICKED && currentPlaylist.ordered) {
                        ...
                        state.onTrackTab.trackThumbnail = getSoundThubmnail(wStrToStr(state.currentSoundFile->path));
                    }
                    lf_pop_style_props();
                }
...
void renderPlaylistAddFromFile() {
    ...
    {
        lf_next_line();
        inputFieldStyled(&state.playlistAddFromFileTab.pathInput.input);
        lf_next_line();
    }
    // Add Button 
    {
...
void renderEditPlaylistPopup() {
    ...
    { 
        ...
        inputFieldStyled(&state.createPlaylistTab.nameInput.input, div_props.color);
        lf_next_line();
    }
    ...
    { 
        ...
        inputFieldStyled(&state.createPlaylistTab.descInput.input, div_props.color);
        lf_next_line();
    }
    lf_next_line();
    {
...
void renderTrackDisplay() {
    ...
    lf_push_font(&state.h6Font);
    float filenameWidth = lf_text_dimension_wide(filename.c_str()).x;
    float filenameHeight = lf_text_dimension_wide(filename.c_str()).y;
    lf_pop_font();

    vec2s containerSize = (vec2s){
        .x = thumbnailContainerSize.x + marginThumbnail + filenameWidth + padding * 2.0f,
        .y = thumbnailContainerSize.y + padding * 2.0f
    };

    vec2s containerPos = (vec2s){
        (float)state.winWidth - containerSize.x - margin,
        (float)state.winHeight - containerSize.y - margin
    };
    lf_rect_render(containerPos, containerSize, lf_color_brightness(GRAY, 0.5), LF_NO_COLOR, 0.0f, 3.0f); 
    ...
    const LfTexture& thumbnail = state.playlists[state.playingPlaylist].musicFiles[state.playlists[state.playingPlaylist].playingFile].thumbnail;
    ...
    lf_text_render_wchar(
            (vec2s){containerPos.x + padding + thumbnailContainerSize.x + marginThumbnail, 
                    containerPos.y + padding + (thumbnailContainerSize.y - filenameHeight) / 2.0f},
...
void renderTrackProgress() {
    ...
    // Progress Bar 
    {
        state.trackProgressSlider.width = 500;
...
void playlistPlayFileWithIndex(uint32_t i, uint32_t playlistIndex) {
    ...
    state.currentSound.init(wStrToStr(playlist.musicFiles[i].path));
    state.currentSound.play();
...
void inputFieldStyled(LfInputField* input, LfColor bgColor) {
    lf_push_style_props(input_field_style(bgColor));
    lf_input_text(input);
    lf_pop_style_props();
}
...
bool playlistFileOrderCorrect(uint32_t playlistIndex, const std::vector<std::string>& paths) {
    ...
    for (size_t i = 0; i < soundFiles.size(); ++i) {
        if (wStrToStr(soundFiles[i].path) != paths[i]) {
            return false;
        }
    }
...
std::vector<SoundFile> reorderPlaylistFiles(const std::vector<SoundFile>& soundFiles, const std::vector<std::string>& paths) {
    ...
    for (size_t i = 0; i < soundFiles.size(); ++i) {
        pathToIndex[wStrToStr(soundFiles[i].path)] = i;
    }

    std::vector<SoundFile> reorderedSoundFiles;

    for (const auto& path : paths) {
        auto it = pathToIndex.find(path);
        if (it != pathToIndex.end()) {
            reorderedSoundFiles.push_back(soundFiles[it->second]);
        }
    }

    return reorderedSoundFiles;
}
